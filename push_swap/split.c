...
static int	count_words(char *s, char c)
		// Esta funçao vai retornar o numero total de substrings de uma string
{
	...
	{
		inside_word = false;     
			// predefinir a flag em falso para iniciar o loop
		while (s[i] == c)        
			// enquanto o caracter atual é igual ao delimitador
			i++;                  // passar para o proximo caracter
		while (s[i] != c && s[i])
			// enquanto o caracter atual for diferente do delimitador e nao chegar ao final da string
		{
			if (!inside_word) // se nao estivermos dentro de uma substring
			{
				count++;           
					// incrementamos o count porque vamos encontrar a nova substring
...
static char	*get_next_word(char *s, char c)
		// Esta função vai retornar uma nova substring
{
	static int cursor = 0;
		// definir o cursor a zeros que vai manter o seu valor durante toda a string
	...
	len = 0;
	while (s[cursor] == c)
		// Percorrer a possibilidade de estarmos em caracteres delimitadores
		cursor++;
	while ((s[cursor + len] != c) && s[cursor + len])
		// Calcular o tamanho da palavra para alocar espaço  para extrair a substring
		++len;
	next_word = malloc((size_t)len * sizeof(char) + 1);
		// Alocar espaço para a substring a ser extraida e para 1 para o caracter nulo
	if (!next_word)                                    
		// verificar se o alocamento foi valido
		return (NULL);
	while ((s[cursor] != c) && s[cursor])
		// percorrer a string e copiar para a nova cada caracter
		next_word[i++] = s[cursor++];
	next_word[i] = '\0'; // igualar o ultimo caracter a nulo
	return (next_word);
}
char	**split(char *s, char c)
		// Esta função vai retonrar as subsstring de uma string separadas por um delimitador
{
	...
	array = malloc(sizeof(char *) * (size_t)(word_count + 2));
		// alocar espaço para o array com o valor do word_count mais 2 slots,
		1 para o caracter nulo da ultima string e outro para o valor nulo do array
	if (!array)                                               
		// verificar se o alocamento foi valido
		return (NULL);
	...
	{
		if (i == 0)
			// caso comece com um delimitador vamos criar a primeira string vazia
		{
		...
		/*char *next_word = get_next_word(s, c);
		if(!next_word)
			return(NULL);*/
		array[i++] = get_next_word(s, c);
			// copiar string a string usando a função get_next_word
	}
